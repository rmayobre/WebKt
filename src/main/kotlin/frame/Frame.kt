package frame

import exception.InvalidFrameException
import java.io.ByteArrayOutputStream

data class Frame(
    /** Is this the final fragment? */
    val isFin: Boolean,
    val rsv1: Boolean,
    val rsv2: Boolean,
    val rsv3: Boolean,
    /** Is the fragment masked? */
    val isMasked: Boolean,
    /** frame.Fragment;s [OpCode] */
    val code: OpCode,
    /** Length of data in fragment. */
    var length: Int,
    /** frame.Fragment's data. */
    val payload: ByteArrayOutputStream

) {
    var next: Frame? = null

    val data: ByteArray
        get() = if (isFin) {
            payload.toByteArray()
        } else {
            next?.let { payload.write(it.payload.toByteArray()) }
            payload.toByteArray()
        }

    val isDataFrame: Boolean = code.isData

    val isControlFrame: Boolean = code.isControl

    /** Construct a dummy Frame. Helps creating the singly linked list. */
    constructor(): this(
        isFin = false,
        rsv1 = false,
        rsv2 = false,
        rsv3 = false,
        isMasked = false,
        code = OpCode.CONTINUATION,
        length = 0,
        payload = ByteArrayOutputStream())

    /**
     * Read first and second byte of a stream from an endpoint into a fragment.
     * @param first - First byte from stream.
     * @param second - Second byte from stream.
     * @throws InvalidFrameException
     * @see <a href="https://tools.ietf.org/html/rfc6455#section-5.2">RFC 6455, Section 5.2 (Base Framing Protocol)</a>
     */
    @Throws(InvalidFrameException::class)
    constructor(first: Int, second: Int): this(
        isFin = first and MASK_FINAL != 0,
        rsv1 = first and MASK_RSV1 != 0,
        rsv2 = first and MASK_RSV2 != 0,
        rsv3 = first and MASK_RSV3 != 0,
        isMasked = second and MASK != 0,
        code = OpCode.find(first and MASK_OPCODE),
        length = second and 0x7F,
        payload = ByteArrayOutputStream())

    init {
        // Run some tests to make sure a valid frame is being constructed.
        if (isControlFrame) {
            if (!isFin)
                throw InvalidFrameException("A control frame must be final; No fragmentation.")
            if (rsv1)
                throw InvalidFrameException("A control frame cannot have rsv1 set to true")
            if (rsv2)
                throw InvalidFrameException("A control frame cannot have rsv2 set to true")
            if (rsv3)
                throw InvalidFrameException("A control frame cannot have rsv3 set to true")
        }
    }

    companion object {
        /** Binary mask to extract the masking flag bit of a WebSocket frame. */
        private const val MASK = 0x80
        /** Binary mask to extract the final fragment flag bit of a WebSocket frame. */
        private const val MASK_FINAL = 0x80
        /** Binary mask to extract RSV1 bit of a WebSocket frame. */
        private const val MASK_RSV1 = 0x40
        /** Binary mask to extract RSV2 bit of a WebSocket frame. */
        private const val MASK_RSV2 = 0x20
        /** Binary mask to extract RSV3 bit of a WebSocket frame. */
        private const val MASK_RSV3 = 0x10
        /** Binary mask to extract the opcode bits of a WebSocket frame. */
        private const val MASK_OPCODE = 0x0F
//        /** Maximum size of Control frame. */
//        private const val MAX_CONTROL_PAYLOAD = 0x7D
    }
}

/*
data class Fragment(
    /** Is this the final fragment? */
    val fin: Boolean,
    val rsv1: Boolean,
    val rsv2: Boolean,
    val rsv3: Boolean,
    /** Is the fragment masked? */
    val isMasked: Boolean,
    /** frame.Fragment;s [frame.OpCode] */
    val code: frame.OpCode,
    /** Length of data in fragment. */
    var length: Int,
    /** frame.Fragment's data. */
    val payload: ByteArrayOutputStream

) {

    /**
     * Create final fragment
     * @param code frame.OpCode of the fragment
     * @param bytes data for the fragment
     * @see frame.OpCode
     * @see <a href="https://tools.ietf.org/html/rfc6455#section-5.2">RFC 6455, Section 5.2 (Base Framing Protocol)</a>
     */
    constructor(code: frame.OpCode, bytes: ByteArray): this(true, false, false, false, false, code, bytes.size, ByteArrayOutputStream(bytes.size)) {
        payload.write(bytes, 0, bytes.size)
    }

    /**
     * Read first and second byte of a stream from an endpoint into a fragment.
     * @param first - First byte from stream.
     * @param second - Second byte from stream.
     * @throws InvalidFrameException
     * @see <a href="https://tools.ietf.org/html/rfc6455#section-5.2">RFC 6455, Section 5.2 (Base Framing Protocol)</a>
     */
    @Throws(InvalidFrameException::class)
    constructor(first: Int, second: Int): this(
        fin = first and MASK_FINAL != 0,
        rsv1 = first and MASK_RSV1 != 0,
        rsv2 = first and MASK_RSV2 != 0,
        rsv3 = first and MASK_RSV3 != 0,
        isMasked = second and MASK != 0,
        code = frame.OpCode.find(first and MASK_OPCODE),
        length = second and 0x7F,
        payload = ByteArrayOutputStream())



    companion object {
        /** Binary mask to extract the masking flag bit of a WebSocket frame. */
        private const val MASK = 0x80
        /** Binary mask to extract the final fragment flag bit of a WebSocket frame. */
        private const val MASK_FINAL = 0x80
        /** Binary mask to extract RSV1 bit of a WebSocket frame. */
        private const val MASK_RSV1 = 0x40
        /** Binary mask to extract RSV2 bit of a WebSocket frame. */
        private const val MASK_RSV2 = 0x20
        /** Binary mask to extract RSV3 bit of a WebSocket frame. */
        private const val MASK_RSV3 = 0x10
        /** Binary mask to extract the opcode bits of a WebSocket frame. */
        private const val MASK_OPCODE = 0x0F
        /** Maximum size of Control frame. */
        private const val MAX_CONTROL_PAYLOAD = 0x7D
    }
}
 */

/*
    /**
     * Constructor to combine multiple frame.Frame fragments into a single WebSocket frame.Frame. Last fragment
     * in list must be the declared as the final fragment.
     * @param fragments Fragmented list of Frames.
     * @throws exception.WebSocketException thrown when the fragmented list does not contain a final fragment of the frame.Frame.
     * @see <a href="https://tools.ietf.org/html/rfc6455#section-5.2">RFC 6455, Section 5.2 (Base Framing Protocol)</a>
     */
	public frame.Frame(List<frame.Frame> fragments) throws exception.WebSocketException {
	    if (fragments.get(fragments.size() - 1).isFin) {
	        isFin = true;
            rsv1 = fragments.get(fragments.size() - 1).rsv1;
            rsv2 = fragments.get(fragments.size() - 1).rsv2;
            rsv3 = fragments.get(fragments.size() - 1).rsv3;
            opcode = fragments.get(fragments.size() - 1).opcode;
            isMasked = fragments.get(fragments.size() - 1).isMasked;
            payload = new ByteArrayOutputStream();
            for (frame.Frame fragment : fragments) {
                try {
                    payload.write(fragment.payload.toByteArray());
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            payload_length = payload.size();
        } else {
	        throw exception.WebSocketException.invalidFrameException("Provided list of frame.Frame fragments did not contain a final fragment.");
        }
    }
     */